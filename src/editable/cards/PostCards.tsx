    ...
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[520px] p-6 sm:p-8 lg:min-h-[620px]">
        ...
        <div className="relative z-10 flex h-full min-h-[460px] flex-col justify-end lg:min-h-[560px]">
          ...
          <h3 className="mt-5 max-w-3xl font-serif text-4xl font-normal leading-[1.02] tracking-normal sm:text-5xl lg:text-6xl">{post.title}</h3>
          <p className="mt-5 max-w-2xl text-sm leading-8 text-white/75 sm:text-base">{getEditableExcerpt(post, 190)}</p>
          <span className={`mt-8 inline-flex w-fit items-center gap-2 border border-white bg-white px-5 py-3 text-xs font-black uppercase tracking-[0.18em] ${pal.panelText}`}>
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        ...
        <span className={`absolute left-4 top-4 ${pal.darkBg} px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-white`}>No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
        <p className={`${dc.type.eyebrow} ${pal.accentText}`}>{getEditableCategory(post)}</p>
        <h3 className={`mt-3 line-clamp-3 font-serif text-2xl font-normal leading-tight tracking-normal ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group block min-w-0 ${dc.surface.soft} p-5 ${dc.motion.lift}`}>
      <div className="flex items-start gap-4">
        <span className={`flex h-11 w-11 shrink-0 items-center justify-center ${pal.darkBg} text-xs font-black text-white`}>{index + 1}</span>
        <div className="min-w-0">
          ...
          <h3 className={`mt-2 line-clamp-2 font-serif text-xl font-normal leading-tight tracking-normal ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`mt-3 line-clamp-3 font-serif text-2xl font-normal leading-tight tracking-normal ${pal.panelText} sm:text-3xl`}>{post.title}</h2>

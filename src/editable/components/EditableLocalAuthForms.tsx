...
const inputClass = 'h-12 border border-[var(--editable-border)] bg-white px-4 text-base font-bold text-[#1a120d] outline-none transition placeholder:text-[#776d63] focus:border-[#1a120d]'
const buttonClass = 'inline-flex h-12 items-center justify-center border border-[#1a120d] bg-[#1a120d] px-6 text-sm font-black uppercase tracking-[0.18em] text-[#fffdf8] transition hover:bg-transparent hover:text-[#1a120d] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-700'}`}>{message}</p> : null}

...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': '#fbf7ef', '--archive-text': '#1a120d', '--archive-surface': '#fffdf8', '--archive-accent': '#9f5f3f', '--editable-border': 'rgba(26,18,13,0.16)', '--editable-container': '1180px' } as CSSProperties
  const categoryLabel = category === 'all' ? 'All categories' : CATEGORY_OPTIONS.find((item) => item.slug === category)?.name || category

  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_340px] lg:px-8 lg:py-16">
          <div className="border-y border-[var(--editable-border)] bg-[var(--archive-surface)] py-8 sm:py-10">
            <div className="inline-flex items-center gap-2 border border-[var(--editable-border)] bg-white/70 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--archive-accent)]"><Icon className="h-4 w-4" /> {label}</div>
            <h1 className="mt-5 max-w-4xl font-serif text-5xl font-normal leading-[1] tracking-normal sm:text-6xl">{voice?.headline || `Browse ${label}`}</h1>
            <p className="mt-6 max-w-2xl text-base leading-8 opacity-70">{voice?.description || SITE_CONFIG.description}</p>
            <div className="mt-6 border-l border-[var(--editable-border)] bg-white/55 p-4 text-sm font-bold leading-7 opacity-75">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="border border-[var(--archive-text)] bg-[var(--archive-text)] px-5 py-3 text-xs font-black uppercase tracking-[0.18em] text-[var(--archive-bg)]">Browse all</Link>
              <Link href="/search" className="border border-[var(--editable-border)] px-5 py-3 text-xs font-black uppercase tracking-[0.18em]">Search articles</Link>
            </div>
          </div>

          <form action={basePath} className="self-end border border-[var(--editable-border)] bg-white/70 p-5 backdrop-blur">
            ...
            <button className="mt-3 h-12 w-full bg-[var(--archive-text)] text-xs font-black uppercase tracking-[0.18em] text-[var(--archive-bg)]">Apply</button>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            ...
            <div className="border border-dashed border-[var(--editable-border)] bg-white/60 p-10 text-center">
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden border border-[var(--editable-border)] bg-white transition hover:-translate-y-1">
      ...
      <div className="p-5">
        ...
        <h2 className="mt-2 font-serif text-2xl font-normal leading-tight tracking-normal">{post.title}</h2>

...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden border border-[var(--editable-border)] bg-white transition hover:-translate-y-1 ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        {!image ? <span className="bg-[var(--editable-page-text,#211713)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
        <h2 className="mt-4 line-clamp-3 font-serif text-2xl font-normal leading-tight tracking-normal text-[var(--editable-page-text,#211713)]">{post.title}</h2>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#fbf7ef)] text-[var(--editable-page-text,#1a120d)]">
        <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 border border-[var(--editable-border)] bg-white/70 p-6 backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] opacity-55">{pagesContent.search.hero.badge}</p>
              <h1 className="mt-5 font-serif text-5xl font-normal leading-[1] tracking-normal sm:text-6xl">{pagesContent.search.hero.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 opacity-70">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-end border border-[var(--editable-border)] bg-[var(--editable-page-bg,#fbf7ef)] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center border border-[#1a120d] bg-[#1a120d] px-6 text-sm font-black uppercase tracking-[0.18em] text-[#fffdf8] transition hover:bg-transparent hover:text-[#1a120d]" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 font-serif text-4xl font-normal tracking-normal">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 border border-dashed border-[var(--editable-border)] bg-white/70 p-10 text-center">
              <p className="font-serif text-3xl">No matching articles found.</p>
